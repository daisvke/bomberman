...
/*   Updated: 2021/08/24 05:42:36 by dtanigaw         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	sl_clear_left_segment(t_env *env, char *map[], int x, int y)
{
	...
	size = env->tex.bomb.explode_size;
	i = 1;
	while (i <= size && (env->tex.bomb.pos.x - i > 0) && map[env->tex.bomb.pos.y][env->tex.bomb.pos.x - i] != MAP_WALL)
	{
...
static void	sl_clear_right_segment(t_env *env, char *map[], int x, int y)
{
	...
	size = env->tex.bomb.explode_size;
	i = 1;
	while (i <= size && (env->tex.bomb.pos.x + i < width) && map[env->tex.bomb.pos.y][env->tex.bomb.pos.x + i] != MAP_WALL)
	{
		sl_replace_with_green_tile(env, x + i * BLOC_LEN, y);
		++i;
	}
