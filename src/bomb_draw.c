...
static void	sl_draw_lower_segment(t_env *env, t_items *bomb, char *map[], \
	t_explode_states states, t_coord pos)
{
	t_coord	coord;
	int		x;
	...
	size = bomb->explode_size;
	i = 1;
	while (i <= (size - 1) && y + i < env->height && map[y + i][x] != MAP_WALL)
	{
		coord = sl_assign_pos(pos.x, pos.y + i * BLOC_LEN);
		sl_render_bloc_with_xpm(&env->canvas, &states.vrt.one, coord, true);
		++i;
	}
	if ((y + i < env->height) && map[y + i][x] != MAP_WALL)
	{
		coord = sl_assign_pos(pos.x, pos.y + i * BLOC_LEN);
...
static void	sl_draw_left_segment(t_env *env, t_items *bomb, char *map[], \
	t_explode_states states, t_coord pos)
{
	t_coord	coord;
	int		x;
...
static void	sl_draw_right_segment(t_env *env, t_items *bomb, char *map[], \
	t_explode_states states, t_coord pos)
{
	t_coord	coord;
	int		x;
	...
	size = bomb->explode_size;
	i = 1;
	while (i <= (size - 1) && (x + i < env->width) && map[y][x + i] != MAP_WALL)
	{
		coord = sl_assign_pos(pos.x + i * BLOC_LEN, pos.y);
		sl_render_bloc_with_xpm(&env->canvas, &states.hrz.one, coord, true);
		++i;
	}
	if ((x + i < env->width) && map[y][x + i] != MAP_WALL)
	{
		coord = sl_assign_pos(pos.x + i * BLOC_LEN, pos.y);
	...
	t_coord pos)
{
	...
	sl_draw_upper_segment(env, bomb, map, bomb_states, pos);
	sl_draw_lower_segment(env, bomb, map, bomb_states, pos);
	sl_draw_left_segment(env, bomb, map, bomb_states, pos);
	sl_draw_right_segment(env, bomb, map, bomb_states, pos);
}
